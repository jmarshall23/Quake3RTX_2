...
void GL_RegisterWorldAreaLight(vec4_t plane, vec3_t mins, vec3_t maxs, int lightStyle, float radius, float r, float g, float b) {
	glLight_t light = { };
	vec3_t origin;
	vec3_t light_clamp;
	...
	light.light_color[0] = plane[0];
	light.light_color[1] = plane[1];
	light.light_color[2] = plane[2];
	light.light_color[3] = plane[3];
