...
float attenuation_arealight(float r, float f, float dist, float3 normal, float3 dir) {
	float angle = dot (dir, normal);
	return r / (dist * dist) * angle;
}
...
{	
	...
	{	 		
		...
		if(lightInfo[i].origin_radius.w > 0) // point lights
		{
			...
			r = (lightInfo[i].origin_radius.w / pow(lightDistance, 1.3)) * angle;
			//r = r * angle;
		}
		else // area lights
		{
  ...
  if(BTriVertex[vertId + 0].st.z != 2 && BTriVertex[vertId + 0].st.z != 3)
  {
	...
	{	 		
		...
		else // area lights
		{
			...
			float4 plane_info = lightInfo[i].light_color;
			...
			float3 centerLightDir = lightInfo[i].origin_radius.xyz - worldOrigin;
			float3 areaLightDir;
